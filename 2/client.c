...
int main (int argc, char* argv[]) {
  ...
  if (sendto(clientSocket, connectionMessage, sizeof(connectionMessage), 0, (struct sockaddr*)NULL, sizeof(hostAddress)) < 0) {
        ...
        if (fds[i].revents != NULL) {
          if (i == 0) { //something in the stdin.
            if((readMessage = read(fileno(stdin), message, maxMessageLength + 1)) == 0) { //read the message, it it fails error out.
              ...
              if (sendto(clientSocket, message, readMessage+1, 0, (struct sockaddr*)NULL, sizeof(hostAddress)) < 0) {
                perror("failed to transmit data.\n");
                exit(EXIT_FAILURE);
              } else {
                //printf("message sent\n");
            ...
            if (recvfrom(clientSocket, message, sizeof(message), 0, (struct sockaddr*)NULL, sizeof(sockaddr)) < 0) { //reading incoming data on the socket. erroing out if something weird happens
